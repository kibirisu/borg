...
export default function MainFeed() {
  ...
  const {
    ...
    opts
      ...
      : {
          queryKey: ["posts-disabled"],
          queryFn: () => Promise.resolve(null),
          enabled: false,
        },
  );

  // Obsługa błędu braku loaderData - po wywołaniu hooka
  if (!loaderData || !opts) {
    return (
      <div className="max-w-2xl mx-auto border-x border-gray-300 min-h-screen bg-white">
        ...
        <div className="p-4 text-center text-red-600">
          Error: Loader data is missing. Please check if the backend endpoint
          /api/posts is implemented.
        </div>
      </div>
    );
  }
